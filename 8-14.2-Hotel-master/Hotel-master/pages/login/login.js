    ...
    wx.checkSession({
      success: function () {
        ...
        wx.login({
          success: res => {
            wx.request({
              url: 'http://dubeniot.com:8080/iot-manager/wx/wxLoginForToken?code='+res.code, //获取code
              data: {
                account: '1514382701',
                jscode: res.code
              },
              ...
              success(res) {
                console.log("4444"+JSON.stringify(res.data));
                if(res.data.status){
                  wx.request({
                    url:'http://dubeniot.com:8080/iot-manager/wx/wxLogin/'+res.data.data,//获取token
                    method: "GET",
                    header: {
                      'content-type': 'application/json' // 默认值
                    },
                    success(res) {
                        console.log("获取信息为："+JSON.stringify(res.data))
                        wx.setStorage({
                          key: "openid",
