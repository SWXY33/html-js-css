...
function pass(player){//玩家选择不出牌
	IsExistFangkuai4(player);//判断是否存在方块4
}		
...
function getSameNum(img1,img2){//根据扑克牌图片img属性获取扑克牌的数值number属性，img1和img2表示出的两张牌的图片路径
var num1,num2;
	for(var i=0;i<arr.length;i++){
		if(arr[i].img==img1){
			num1=arr[i].number;
		}else if(arr[i].img==img2){
			num2=arr[i].number;
		}
	}
	console.log("num1="+num1+"num2="+num2);
	if(num1==num2){
		console.log("两张牌数值相同，出的是对子，符合规则，可以出牌。");
		return 1;
	}else{
		console.log("选的牌不符合规则，请重新选牌！");
		return 0;
	}
}
function VerifyTwo(player){//验证出牌两张数值是否相同,player为当前出牌玩家
var img=[];
$('img').each(function(){
...
console.log("2选中的扑克牌的图片是："+img);
	if(findFangkuai4()==player&&getSameNum(img[0],img[1])==1){
		...
		}else if(findFangkuai4()!=player&&getSameNum(img[0],img[1])==1){
		return 1;
	}else if(findFangkuai4()!=player&&getSameNum(img[0],img[1])==0){
		return 0;
	}else{
			return 0;
			}	
...
function removepokers(player,image){//根据img删除的数组元素，数组player表示玩家的手牌A[]、B[]、C[]、D[]
var gameplayer=[],deletearr=[];
    var role;
	if(player=="A"){
		player=A;role="A";
	}else if(player=="B"){
	...
	for(var i=0;i<player.length;i++){
		gameplayer.push(player[i].img);
		if(player[i].img==image){
			player.splice(i,1);
		}
	}
	console.log(role+"删除前的数组是："+gameplayer);
	for(var i=0;i<player.length;i++){
		deletearr.push(player[i].img);
	}
	console.log(role+"删除后的数组是："+deletearr);
	return deletearr;
}
function ComparePokers(){//比较牌面大小
...
var pokers=[];//当前玩家手牌
	...
	if(lastplayer=="A"){last_player="A";}
	else if(lastplayer=="B"){last_player="B";}
	else if(lastplayer=="C"){last_player="C";}
	else if(lastplayer=="D"){last_player="D";}
	function getLastOutPokers(){//获取上家出的牌
	console.log("22222222")
	findTimes=0;
	lastplayer=FindLastOutPokers(last_player);
	console.log("findTimesfindTimesfindTimes="+findTimes);
	var last_out_pokers=$("#out_"+lastplayer+" .out_pokers");
		...
		console.log("111追溯到最近出牌玩家"+lastplayer+"出的牌是："+last_out);
		console.log("66666追溯到最近出牌玩家出的牌："+last_out.length+"当前玩家出牌："+out.length);
	if(findTimes>3){
		console.log("666666666666666666666666666666666666666你获得了优先出牌权");
		return 1;
		}else{
...
function out_pokers(){
		...
		for(var i=0;i<out.length;i++){
			...
			$('button[class^="operate1"]').remove();//隐藏当前玩家出牌操作，或$('button').remove('.operate1');
			ShowOperate(nextplayer);//显示下家出牌操作区
...
function FindLastOutPokers(last_player){//追溯最近一次玩家出牌
if(findTimes>3){
	console.log("其他人都不要！你获得任意出牌权");
	...
	console.log("追溯的玩家是============"+last_player);
			var list=$("#out_"+last_player+" .out_pokers");
			for(var i=0;i<list.length;i++){
				console.log("上家出的牌：：：：：：："+list[i].innerHTML);
				if(list[i].innerHTML=="<div class=\"PASS\">不出</div>"){
					console.log("****判断为：上家不出牌，再往后追溯上一家出的牌");
					if(last_player=="A"){last_player="D"; findTimes++;console.log("追溯次数为："+findTimes);console.log("A没有出牌，再往后追溯到D");return FindLastOutPokers(last_player);}
					else if(last_player=="B"){last_player="C";findTimes++;console.log("追溯次数为："+findTimes);console.log("B没有出牌，再往后追溯到C");return FindLastOutPokers(last_player);}
					else if(last_player=="C"){last_player="A";findTimes++;console.log("追溯次数为："+findTimes);console.log("C没有出牌, 再往后追溯到A");return FindLastOutPokers(last_player);}
					else if(last_player=="D"){last_player="B";findTimes++;console.log("追溯次数为："+findTimes);console.log("D没有出牌, 再往后追溯到B");return FindLastOutPokers(last_player);}
			    }else{
					findTimes++;
					console.log("已追溯到最近一次出牌的玩家："+last_player);
					console.log("追溯次数为："+findTimes);
...
function Out_Pokers(){
		...
		$("#out_"+FindLastOutPokers(last_player)+" img").each(function(){
		...
		if(out.length==1){
			...
			if(last_out_type==undefined&&last_out_number==undefined){last_out_type=0;last_out_number=0;}
			...
			if(out_type==last_out_type&&out_number>last_out_number){//花色相同，当前出牌数值比上家大
				...
				out_pokers();
				return 1;
			}else if(last_out_type==0&&last_out_number==0){//上家没有出牌或者没有人要当前玩家的牌
			console.log("你获得优先出牌权");
				out_pokers();
				return 1;
			}else{
			...
			console.log("——————出两张牌————————");
				for(var j=0;j<out_img.length;j++){
					for(var i=0;i<arr.length;i++){
						if(out_img[j]==arr[i].img){
							out_type[j]=arr[i].type;
							out_number[j]=arr[i].number;
						}else if(last_out_img[j]==arr[i].img){
							last_out_type[j]=arr[i].type;
							last_out_number[j]=arr[i].number;
						}
					}
				}
			console.log("当前出两张牌的type0="+out_type[0]+"；number0="+out_number[0]+"type1="+out_type[1]+"；number1="+out_number[1]);
			console.log("上家出牌的type="+last_out_type+"；number="+last_out_number);
			if(out_number[0]==out_number[1]&&last_out_type.length==0){
				out_pokers();
				console.log("你先出牌，可以随便出对子")
				return 1;
			}else if(out_number[0]==out_number[1]&&last_out_number[0]==last_out_number[1]&&out_number[0]>last_out_number[0]
			||out_number[0]==out_number[1]&&last_out_number[0]==last_out_number[1]&&out_number[0]==last_out_number[0]&&out_type.indexOf(4)){
				out_pokers();
				console.log("出的对子比上家大")
				return 1;
			}else{
			console.log("您选牌不符合规则，请重新选牌！");
			return;
	...
	function outpokers(){
					 ...
					 if(getLastOutPokers()==1){
					 	console.log("222与上家出牌数量一致0000");
					     console.log(player+"已经出过牌！");
					     
						 console.log("findTimes==========="+findTimes)
						 if(findTimes>3||findTimes<=3&&Out_Pokers()==1){
							return 1; 
						 }
				 }else{
					return 0;
	...
	if(out.length!=0){
		if(out.length==1){//如果出1张牌
			for(var x=0;x<out.length;x++){
				if(findFangkuai4()==player&&out[x]=="<img id=\"43.jpg\" src=\"img/43.jpg\">"||findFangkuai4()!=player){//如果player手牌有♦4而且出的牌是它、或者当前玩家手牌没有方块4
						if(outpokers()==1){
							console.log("3与上家出牌数量一致");
							//outpokers();
						}else{
							console.log("3出牌不符合规则");
							return;
						}
					}else{
				console.log("请先出♦4，请重新选牌！");
				return;
				}
			}
		}else if(out.length==2){//如果出2张牌
		if(VerifyTwo(player)==1){
			if(outpokers()==1){
				console.log("4与上家出牌数量一致");
				//outpokers();
			}else{
				console.log("4与上家出牌数量bu一致");
				return;
			}
	    }else{
				console.log("请先出♦4，请重新选牌！");
		...
		if(findFangkuai4()==player&&out[x]=="<img id=\"43.jpg\" src=\"img/43.jpg\">"||findFangkuai4()!=player){
