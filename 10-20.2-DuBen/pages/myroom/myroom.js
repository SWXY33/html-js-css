  ...
  onLoad: function (options) {
    ...
    wx.request({//查询订单 
      ...
      success(res) { 
        ...
        for(var i=0;i<myorder.length-1;i++){
        ...
        console.log("冒泡排序前：myorder==="+JSON.stringify(myorder))
        //冒泡排序法
        for(var i=0;i<myorder.length-1;i++){
          for(var j=0;j<myorder.length-1-i;j++){
            //相邻元素两两比对，元素交换，大的在前
            var nT = Date.parse(myorder[j]["ordertime"].replace(/-/g,"/"));
            var nT1 = Date.parse(myorder[j+1]["ordertime"].replace(/-/g,"/"));
             if(nT<nT1){
              var temp = myorder[j];
        ...
        console.log("冒泡排序后：myorder==="+JSON.stringify(myorder))
      that.setData({
        orderData:myorder
      })
      }
